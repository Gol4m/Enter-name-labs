...
class Interface(App):
    ...
    def menu_choose_table(self, obj=None):  #
        ...
        buttons_create_and_list_tabels = GridLayout(cols=2, size_hint=(1, .6),
    ...
    def check_list_tabel(self, obj=None):  #
        ...
        table_buttons_widget = BoxLayout(orientation='vertical', size_hint=(1, .8),
                                         pos_hint={'center_x': .5, 'center_y': .5})
        count_of_file_xml = 0
    ...
    def generate_table_widget(self):  #
        container_table_and_pages = BoxLayout(orientation='vertical', size_hint=(1, .88),
    ...
    def generate_search_record_widget(self, obj=None):
        ...
        self.table_search_select_title = Spinner(text="Select title",
                                                 values=(
                                                 'Faculty', 'Department', 'FCs', 'Academic title', 'Academic degree',
                                                 'Work experience'),
                                                 font_size='15sp')
    ...
    def table_search_ui(self, obj=None):
        ...
        label = Label(text="Найдено элементов: {}".format(int(len(self.found_records))),
    ...
    def generate_table_search_widget(self):  #
        ...
        for index in range((self.current_pages_popup_search - 1) * self.records_on_page,
                           self.current_pages_popup_search * self.records_on_page):
        ...
        pages_buttons.add_widget(button_page_previous)
        # add pages
        pages_buttons.add_widget(
            Label(text="{}/{}".format(self.current_pages_popup_search, self.count_of_all_pages_for_search())))
        pages_buttons.add_widget(button_page_next)
        pages_buttons.add_widget(button_page_last)
    ...
    def table_tool_add_record(self, obj=None):  #
        ...
        popup.open()
        closeButton.bind(on_press=popup.dismiss)
        screen_popup.add_widget(
            Label(text="added successfully", font_size='15sp', pos_hint={'center_x': .5, 'center_y': .75}))
    ...
    def generate_save_widget(self, obj=None):
        ...
        closeButton.bind(on_press=popup.dismiss)
        screen_popup.add_widget(
            Label(text="saved successfully", font_size='15sp', pos_hint={'center_x': .5, 'center_y': .75}))
        screen_popup.add_widget(closeButton)
